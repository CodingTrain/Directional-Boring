...
function createHddScene() {
    ...
    hddScene.triangle(
        goal.x - 6,
        groundLevel - goal.w - 2,
        goal.x + goal.w + 6,
        groundLevel - goal.w - 2,
        goal.x + goal.w / 2,
        groundLevel - goal.w * 1.8
    );
}
...
function updateDivWithLinkToThisLevel() {
    seedDiv.html(
        '<a href="?seed=' + currentSeed + '">Persistent link to THIS level</a>'
    );
}
...
function pullBack() {
    if (state == "PAUSED" || state == "DRILLING" || state == "STUCK") {
        state = "PAUSED";
        let prevPosition =
            Math.floor((pathPosition - 1) / pipeLength) * pipeLength;
        if (prevPosition > 0) {
...
function drawReflection(reflectionImage) {
    ...
    for (let x = 0; x < width - spacing; x += step) {
        ...
        let distToObjWithNoize =
            (((100 + random(-10, 10)) / 100) * minTravelDist) / 2;
        let xMid = x + spacing / 2;
        reflectionImage.fill(125);
        reflectionImage.noStroke();
        reflectionImage.circle(
            xMid,
            distToObjWithNoize + groundLevel,
            visualRad
        );
    }
...
function draw() {
    ...
    if (state == "STUCK") {
        ...
        textFont("courier");
        text(
            "STUCK! (" + stuckCount + "/" + maxStuckTimes + " times)",
            width / 2,
            groundLevel / 2
        );
    }
